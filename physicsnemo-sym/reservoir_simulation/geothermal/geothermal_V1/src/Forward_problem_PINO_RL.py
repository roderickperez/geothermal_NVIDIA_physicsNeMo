...
# Default to Reverse A (safe default)
@physicsnemo.sym.main(config_path="conf", config_name="config_PINO_M4_Reverse_A")
def run(cfg: PhysicsNeMoConfig) -> None:
    print("\n|-----------------------------------------------------------------|")
    print("|    PINO MODEL 4: REVERSE LEARNING (Physics -> Data)             |")
    print("|-----------------------------------------------------------------|")
    ...
    nv = cfg.custom.NVRS
    geothermal_physics = Geothermal_Physics(nv.UIR, nv.pini_alt, tini_alt=nv.tini_alt, LUB=nv.LUB, HUB=nv.HUB, aay=nv.aay, bby=nv.bby, MAXZ=nv.MAXZ, nx=nv.nx, ny=nv.ny, nz=nv.nz)
