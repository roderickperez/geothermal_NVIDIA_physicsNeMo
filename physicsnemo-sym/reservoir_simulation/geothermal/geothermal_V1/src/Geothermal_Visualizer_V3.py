...
def load_data_and_model(checkpoint_path, data_path, device):
    ...
    try:
        ...
        print("V3 Delta Models loaded successfully.")
...
def plot_slice(model_name, true_vol, pred_vol, layer, var_name, out_dir, dynamic_wells, idx, dataset_type):
    ...
    plt.suptitle(f"{model_name} V3 HighFreq | {dataset_type} Data (Idx {idx}) | {var_name} | Final Step", fontsize=18, fontweight='bold')
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, f"V3_Viz_{dataset_type}_{var_name}_Idx{idx}.png"), dpi=150)
    plt.close()
...
if __name__ == "__main__":
    ...
    if mod_p:
        ...
        print(f"Saved highly-formatted V3 physical plots for {dataset_type} data to {out_dir}")