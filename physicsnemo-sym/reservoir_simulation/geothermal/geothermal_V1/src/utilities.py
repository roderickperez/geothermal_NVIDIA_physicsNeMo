...
def load_FNO_dataset(path, input_keys, output_keys, n_examples=None):
    ...
    for d, keys in [(invar, input_keys), (outvar, output_keys)]:
        for k in keys:
            # get data
            x = data[k][:]  # N, C, H, W (Slice to load into memory)
...
def load_FNO_dataset2(path, input_keys, output_keys, output_keys2, n_examples=None):
    ...
    for d, keys in [
        ...
        for k in keys:
            ...
            # --- GEOTHERMAL NORMALIZATION (V7 UPDATED: Scientific Perfection) ---
            if k in input_keys or k == "pressure" or k == "temperature":
                # 1. Log-Scale Permeability
                if k == "perm":
                    x = np.log10(x)
                
                # 2. Sparse Input Scaling (Preserves Sparsity)
                elif k == "Q":
                    x = x / 2000.0  # Normalize Rate (1500 becomes 0.75, 0 stays 0)
                elif k == "Qw":
                    x = x / 373.0   # Normalize Inj Temp (303K becomes 0.81, 0 stays 0)
                
                # 3. Dynamic Field Normalization (Shift & Scale)
                elif k == "Tini":  # Initial Reservoir Temp
                    x = (x - 273.0) / 100.0 # Map 373K -> 1.0
                elif k == "temperature": # Output Temperature (Mapped from Kelvin)
                    x = (x - 273.0) / 100.0 # Map 300-373K -> 0.27-1.0
                elif k == "pressure":
                    x = (x - 100.0) / 200.0 # Map 150-300 bar -> ~0.25-1.0
            # -------------------------------------------------------------------
            # -------------------------------------------
