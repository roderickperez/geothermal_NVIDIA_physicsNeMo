...
def load_fno_model(checkpoint_dir, device):
    print(f"Loading FNO from {checkpoint_dir}...")
    steppi = 30
    input_keys = ["perm", "Q", "Qw", "Phi", "Time", "Pini", "Tini"]
    ...
    if os.path.exists(p_path):
        ...
        print("Could not find FNO weights.")
        return None
    return model_p
...
if __name__ == "__main__":
    ...
    try:
        ...
        draw_map(axes[0,1], pred_tr, "TRAINING FNO Prediction", q_train)
        draw_map(axes[0,2], np.abs(true_tr - pred_tr), "TRAINING Absolute Error", q_train, is_diff=True)
    except Exception as e:
        print(f"Error drawing training map: {e}")

    # ROW 2: TEST SET
    try:
        draw_map(axes[1,0], true_te, "TEST Ground Truth", q_test)
        draw_map(axes[1,1], pred_te, "TEST FNO Prediction", q_test)
        draw_map(axes[1,2], np.abs(true_te - pred_te), "TEST Absolute Error", q_test, is_diff=True)
    except Exception as e:
        print(f"Error drawing test map: {e}")

    plt.suptitle("FNO Generalization Check: Dynamic True Wells View\n(Layer 2 Map View - Final Timestep)", fontsize=16, fontweight='bold')
    plt.tight_layout()
    
    save_path = os.path.join(out_dir, "Overfitting_Analysis_FNO.png")
